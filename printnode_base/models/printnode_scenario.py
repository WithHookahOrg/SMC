...
class PrintNodeScenario(models.Model):
    ...
    def print_reports(self, action, model_id, ids_list):
        ...
        for scenario in scenarios:
            ...
            if objects.exists():
                if hasattr(self.env[scenario.model_id.model], f'_scenario_{scenario.action.code}'):
                    scenario_method = getattr(objects, f'_scenario_{scenario.action.code}')
                    scenario_method(report_id=scenario.report_id)
                else:
    ...
    def _apply_domain(self, ids_list):
        ...
        if self.domain == '[]':
            return self.env[self.reports_model_id.model].browse(ids_list)
        return self.env[self.reports_model_id.model].search(
...
class PrintNodeScenarioMixin(models.AbstractModel):
    ...
    def print_scenarios(self, action, ids_list=None):
        """
